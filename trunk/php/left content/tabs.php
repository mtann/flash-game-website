<?php
	...
	switch($p) {
		...
		case "3": 
			...
			$result = mysql_query("select game.id, game.avatar, game.name, game.totalplay from game inner join vote on game.id=vote.game_id  where game.type='$type' order by (vote.total_good+vote.total_bad) desc limit 20");
